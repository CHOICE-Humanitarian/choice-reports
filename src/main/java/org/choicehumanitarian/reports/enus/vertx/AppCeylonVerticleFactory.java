 ...
 */
public class AppCeylonVerticleFactory implements VerticleFactory {

	private static final String VERSION;

	static {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		try (InputStream in = AppCeylonVerticleFactory.class.getResourceAsStream("/vertx-ceylon-version.txt")) {
			if (in != null) {
				while (true) {
					int i = in.read();
					if (i == -1) {
	...
	public AppCeylonVerticleFactory() {
	}
	...
	public Verticle createVerticle(String verticleName, ClassLoader classLoader) throws Exception {
		...
		if (verticleName.endsWith(".ceylon")) {
			...
			if (moduleRoot != null) {
				...
				synchronized (modules) {
					...
					if (module == null) {
						...
						JavaRunner runner = (JavaRunner) CeylonToolProvider.getRunner(Backend.Java, runnerOptions,
								"io.vertx.ceylon.core", VERSION);
						modules.put(moduleName,
								module = new Module(compiledModule.name, compiledModule.version, runner));
					} else {
						module.instances++;
					}
				}
			...
			return new AppCeylonVerticle(module);
		}
	}
	...
	private List<ModuleInfo> compileModules(File sourcePath, List<File> sources) throws Exception {
		...
		boolean compiled = compiler.compile(options, new CompilationListener() {
			...
			public void error(File file, long line, long column, String message) {
				String msg;
				if (file != null) {
					msg = "Compilation error at (" + line + "," + column + ") in " + file.getAbsolutePath() + ":"
							+ message;
				} else {
			...
			public void warning(File file, long line, long column, String message) {
				String msg;
				if (file != null) {
					msg = "Compilation warning at (" + line + "," + column + ") in " + file.getAbsolutePath() + ":"
							+ message;
				} else {
	...
	public void createVerticle(String verticleName, ClassLoader classLoader, Promise<Callable<Verticle>> promise) {
		try {
			promise.complete(() -> createVerticle(verticleName, classLoader));
		} catch (Exception ex) {
			promise.fail(ex);
		}
	}
