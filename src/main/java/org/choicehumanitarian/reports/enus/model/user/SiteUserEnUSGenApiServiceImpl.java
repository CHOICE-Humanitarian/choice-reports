...
import org.choicehumanitarian.reports.enus.model.user.SiteUserPage;
				...
				switch(entityVar) {
					...
					case "setNarrativeKeys":
						...
						Optional.ofNullable(o.getNarrativeKeys()).orElse(Arrays.asList()).stream().filter(oVal -> oVal != null && !setNarrativeKeysValues.contains(oVal.toString())).forEach(pk2 -> {
							...
							futures2.add(Future.future(promise2 -> {
								sql(siteRequest).update(ReportNarrative.class, pk2).setToNull(ReportNarrative.VAR_assigneeKey, SiteUser.class, pk2).onSuccess(a -> {
									promise2.complete();
								}).onFailure(ex -> {
									promise2.fail(ex);
								});
					...
					case "setEventKeys":
						...
						Optional.ofNullable(o.getEventKeys()).orElse(Arrays.asList()).stream().filter(oVal -> oVal != null && !setEventKeysValues.contains(oVal.toString())).forEach(pk2 -> {
							...
							futures2.add(Future.future(promise2 -> {
								sql(siteRequest).update(ReportEvent.class, pk2).setToNull(ReportEvent.VAR_assigneeKey, SiteUser.class, pk2).onSuccess(a -> {
									promise2.complete();
								}).onFailure(ex -> {
									promise2.fail(ex);
								});
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("choice-reports-enUS-SiteUser", json, new DeliveryOptions().addHeader("action", "putimportSiteUserFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
		...
		try {
			...
			sqlConnection.preparedQuery("SELECT pk as pk1, 'narrativeKeys' from ReportNarrative where assigneeKey=$1 UNION SELECT pk as pk1, 'eventKeys' from ReportEvent where assigneeKey=$2 UNION SELECT pk as pk1, 'pullOwnerReportScheduleKeys' from ReportSchedule where pullOwnerKey=$3 UNION SELECT pk as pk1, 'finalOwnerReportScheduleKeys' from ReportSchedule where finalOwnerKey=$4")
				...
				try {
					...
					LOG.error(String.format("relateSiteUser failed. "), ex);
					promise.fail(ex);
				}
			}).onFailure(ex -> {
				RuntimeException ex2 = new RuntimeException(ex);
