...
import org.choicehumanitarian.reports.enus.model.htm.SiteHtmPage;
		...
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, "choice-reports-enUS-SiteUser", "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				if(
					...
					eventBus.request("choice-reports-enUS-SiteHtm", json, new DeliveryOptions().addHeader("action", "postSiteHtmFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("choice-reports-enUS-SiteHtm", json, new DeliveryOptions().addHeader("action", "putimportSiteHtmFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
