...
public class SitePageGenPage extends SitePageGenPageGen<PageLayout> {
	...
	protected void _varsQ(JsonObject vars) {
		SitePage.varsQForClass().forEach(var -> {
			...
			json.put("val", Optional.ofNullable(searchListSitePage_.getRequest().getQuery()).filter(fq -> fq.startsWith(SitePage.varIndexedSitePage(var) + ":")).map(s -> StringUtils.substringAfter(s, ":")).orElse(null));
			vars.put(var, json);
		});
	}
