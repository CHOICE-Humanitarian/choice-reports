...
public class ChoiceDonorEnUSGenApiServiceImpl extends BaseApiServiceImpl implements ChoiceDonorEnUSGenApiService {
	...
	public ChoiceDonorEnUSGenApiServiceImpl(EventBus eventBus, JsonObject config, WorkerExecutor workerExecutor, PgPool pgPool, WebClient webClient, OAuth2Auth oauth2AuthenticationProvider, AuthorizationProvider authorizationProvider) {
	...
	public Future<ChoiceDonor> postChoiceDonorFuture(SiteRequestEnUS siteRequest, Boolean inheritPk) {
		...
		try {
			pgPool.withTransaction(sqlConnection -> {
				...
				createChoiceDonor(siteRequest).onSuccess(choiceDonor -> {
					sqlPOSTChoiceDonor(choiceDonor, inheritPk).onSuccess(b -> {
						defineChoiceDonor(choiceDonor).onSuccess(c -> {
							attributeChoiceDonor(choiceDonor).onSuccess(d -> {
				...
				LOG.debug(String.format("postChoiceDonorFuture succeeded. "));
			}).onFailure(ex -> {
				promise.fail(ex);
				error(siteRequest, null, ex);
			});
	...
	public Future<ChoiceDonor> patchChoiceDonorFuture(ChoiceDonor o, Boolean inheritPk) {
		...
		try {
			...
			pgPool.withTransaction(sqlConnection -> {
				...
				promise.complete(choiceDonor);
				LOG.debug(String.format("patchChoiceDonorFuture succeeded. "));
			}).onFailure(ex -> {
				promise.fail(ex);
				error(siteRequest, null, ex);
			});
		...
		user(serviceRequest).onSuccess(siteRequest -> {
			try {
				...
				if(
					...
					searchChoiceDonorList(siteRequest, false, true, false, "/api/donor/{id}", "GET").onSuccess(listChoiceDonor -> {
		...
		user(serviceRequest).onSuccess(siteRequest -> {
			try {
				...
				if(
					...
					searchChoiceDonorList(siteRequest, false, true, false, "/api/donor", "Search").onSuccess(listChoiceDonor -> {
	...
	public Future<ChoiceDonor> createChoiceDonor(SiteRequestEnUS siteRequest) {
		...
		try {
			...
			sqlConnection.preparedQuery("INSERT INTO ChoiceDonor(created, userKey) VALUES($1, $2) RETURNING pk")
				...
				LOG.error("createChoiceDonor failed. ", ex);
				promise.fail(ex);
			});
		} catch(Exception ex) {
	...
	public void searchChoiceDonorQ(String uri, String apiMethod, SearchList<ChoiceDonor> searchList, String entityVar, String valueIndexed, String varIndexed) {
		searchList.setQuery(varIndexed + ":" + ("*".equals(valueIndexed) ? valueIndexed : ClientUtils.escapeQueryChars(valueIndexed)));
		if(!"*".equals(entityVar)) {
		}
	}

	public String searchChoiceDonorFq(String uri, String apiMethod, SearchList<ChoiceDonor> searchList, String entityVar, String valueIndexed, String varIndexed) {
		if(varIndexed == null)
	...
	public void searchChoiceDonorSort(String uri, String apiMethod, SearchList<ChoiceDonor> searchList, String entityVar, String valueIndexed, String varIndexed) {
		if(varIndexed == null)
			throw new RuntimeException(String.format("\"%s\" is not an indexed entity. ", entityVar));
		searchList.addSort(varIndexed, ORDER.valueOf(valueIndexed));
	}

	public void searchChoiceDonorRows(String uri, String apiMethod, SearchList<ChoiceDonor> searchList, Integer valueRows) {
			searchList.setRows(apiMethod != null ? valueRows : 10);
	}

	public void searchChoiceDonorStart(String uri, String apiMethod, SearchList<ChoiceDonor> searchList, Integer valueStart) {
		searchList.setStart(valueStart);
	}

	public void searchChoiceDonorVar(String uri, String apiMethod, SearchList<ChoiceDonor> searchList, String var, String value) {
		searchList.getSiteRequest_().getRequestVars().put(var, value);
	}

	public void searchChoiceDonorUri(String uri, String apiMethod, SearchList<ChoiceDonor> searchList) {
	}
	...
	public Future<SearchList<ChoiceDonor>> searchChoiceDonorList(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod) {
		...
		try {
						...
						for(Object paramObject : paramObjects) {
							switch(paramName) {
								...
								case "fq":
									...
									if(foundFq) {
										...
										while(foundFq) {
											...
											String entityFq = searchChoiceDonorFq(uri, apiMethod, searchList, entityVar, valueIndexed, varIndexed);
								...
								case "var":
									entityVar = StringUtils.trim(StringUtils.substringBefore((String)paramObject, ":"));
									valueIndexed = URLDecoder.decode(StringUtils.trim(StringUtils.substringAfter((String)paramObject, ":")), "UTF-8");
									searchChoiceDonorVar(uri, apiMethod, searchList, entityVar, valueIndexed);
									break;
							}
						}
						searchChoiceDonorUri(uri, apiMethod, searchList);
					}
				} catch(Exception e) {
			...
			searchChoiceDonor2(siteRequest, populate, store, modify, uri, apiMethod, searchList);
			searchList.promiseDeepForClass(siteRequest).onSuccess(a -> {
	...
	public void searchChoiceDonor2(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod, SearchList<ChoiceDonor> searchList) {
	}
	...
	public Future<Void> attributeChoiceDonor(ChoiceDonor o) {
		Promise<Void> promise = Promise.promise();
			promise.complete();
		return promise.future();
	}
