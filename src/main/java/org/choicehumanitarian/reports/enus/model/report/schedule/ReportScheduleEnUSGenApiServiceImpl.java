...
import org.choicehumanitarian.reports.enus.model.report.schedule.ReportSchedulePage;
...
public class ReportScheduleEnUSGenApiServiceImpl extends BaseApiServiceImpl implements ReportScheduleEnUSGenApiService {
	...
	public void putimportReportScheduleFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, "choice-reports-enUS-SiteUser", "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				searchList.fq("inheritPk_docvalues_string:" + SearchTool.escapeQueryChars(body.getString(ReportSchedule.VAR_pk)));
						...
						if(searchList.size() >= 1) {
							...
							if(body2.size() > 0) {
								...
								patchReportScheduleFuture(o, true).onSuccess(b -> {
									LOG.info("Import ReportSchedule {} succeeded, modified ReportSchedule. ", body.getValue(ReportSchedule.VAR_pk));
									eventHandler.handle(Future.succeededFuture());
								}).onFailure(ex -> {
							...
							postReportScheduleFuture(siteRequest, true).onSuccess(b -> {
								LOG.info("Import ReportSchedule {} succeeded, created new ReportSchedule. ", body.getValue(ReportSchedule.VAR_pk));
								eventHandler.handle(Future.succeededFuture());
							}).onFailure(ex -> {
								LOG.error(String.format("putimportReportScheduleFuture failed. "), ex);
								eventHandler.handle(Future.failedFuture(ex));
			...
			pgPool.withTransaction(sqlConnection -> {
				...
				createReportSchedule(siteRequest).onSuccess(reportSchedule -> {
					sqlPOSTReportSchedule(reportSchedule, inheritPk).onSuccess(b -> {
						persistReportSchedule(reportSchedule).onSuccess(c -> {
			...
			if(jsonObject != null) {
				...
				for(String entityVar : entityVars) {
					...
					case ReportSchedule.VAR_eventKeys:
						...
						break;
					case ReportSchedule.VAR_pullOwnerKey:
						Optional.ofNullable(jsonObject.getString(entityVar)).ifPresent(val -> {
							futures1.add(Future.future(promise2 -> {
								search(siteRequest).query(SiteUser.class, val, inheritPk).onSuccess(pk2 -> {
									...
									sql(siteRequest).update(ReportSchedule.class, pk).set(ReportSchedule.VAR_pullOwnerKey, SiteUser.class, pk2).onSuccess(a -> {
										promise2.complete();
									}).onFailure(ex -> {
										promise2.fail(ex);
									});
						...
						break;
					case ReportSchedule.VAR_finalOwnerKey:
						Optional.ofNullable(jsonObject.getString(entityVar)).ifPresent(val -> {
							futures1.add(Future.future(promise2 -> {
								search(siteRequest).query(SiteUser.class, val, inheritPk).onSuccess(pk2 -> {
									if(!pks.contains(pk2)) {
									...
									sql(siteRequest).update(ReportSchedule.class, pk).set(ReportSchedule.VAR_finalOwnerKey, SiteUser.class, pk2).onSuccess(a -> {
										promise2.complete();
									}).onFailure(ex -> {
										promise2.fail(ex);
									});
					...
					case ReportSchedule.VAR_firstDueDate:
						...
						break;
					case ReportSchedule.VAR_dataPeriodStartDate:
						o2.setDataPeriodStartDate(jsonObject.getString(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
						bSql.append(ReportSchedule.VAR_dataPeriodStartDate + "=$" + num);
						num++;
						bParams.add(o2.sqlDataPeriodStartDate());
						break;
					case ReportSchedule.VAR_dataPullEndDate:
						o2.setDataPullEndDate(jsonObject.getString(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
						bSql.append(ReportSchedule.VAR_dataPullEndDate + "=$" + num);
						num++;
						bParams.add(o2.sqlDataPullEndDate());
						break;
				...
				if(
					...
					searchReportScheduleList(siteRequest, false, true, true).onSuccess(listReportSchedule -> {
						try {
							...
							if(listReportSchedule.getResponse().getResponse().getNumFound() > 1
								...
								apiRequest.setNumFound(listReportSchedule.getResponse().getResponse().getNumFound());
								apiRequest.setNumPATCH(0L);
								apiRequest.initDeepApiRequest(siteRequest);
								siteRequest.setApiRequest_(apiRequest);
	...
	public Future<Void> listPATCHReportSchedule(ApiRequest apiRequest, SearchList<ReportSchedule> listReportSchedule) {
		...
		CompositeFuture.all(futures).onSuccess( a -> {
			if(apiRequest != null) {
				apiRequest.setNumPATCH(apiRequest.getNumPATCH() + listReportSchedule.getResponse().getResponse().getDocs().size());
				if(apiRequest.getNumFound() == 1L)
			...
			listReportSchedule.next().onSuccess(next -> {
				if(next) {
					listPATCHReportSchedule(apiRequest, listReportSchedule).onSuccess(b -> {
						promise.complete();
					}).onFailure(ex -> {
						LOG.error(String.format("listPATCHReportSchedule failed. "), ex);
						promise.fail(ex);
			...
			try {
				...
				searchReportScheduleList(siteRequest, false, true, true).onSuccess(listReportSchedule -> {
					try {
						...
						if(o != null && listReportSchedule.getResponse().getResponse().getNumFound() == 1) {
							ApiRequest apiRequest = new ApiRequest();
							apiRequest.setRows(1L);
							apiRequest.setNumFound(1L);
							apiRequest.setNumPATCH(0L);
				...
				switch(entityVar) {
					...
					case "removePullOwnerKey":
						Optional.ofNullable(jsonObject.getString(entityVar)).map(val -> Long.parseLong(val)).ifPresent(pk2 -> {
							...
							futures2.add(Future.future(promise2 -> {
								sql(siteRequest).update(ReportSchedule.class, pk).setToNull(ReportSchedule.VAR_pullOwnerKey, SiteUser.class, pk2).onSuccess(a -> {
									promise2.complete();
								}).onFailure(ex -> {
									promise2.fail(ex);
								});
					...
					case "removeFinalOwnerKey":
						Optional.ofNullable(jsonObject.getString(entityVar)).map(val -> Long.parseLong(val)).ifPresent(pk2 -> {
							...
							futures2.add(Future.future(promise2 -> {
								sql(siteRequest).update(ReportSchedule.class, pk).setToNull(ReportSchedule.VAR_finalOwnerKey, SiteUser.class, pk2).onSuccess(a -> {
									promise2.complete();
								}).onFailure(ex -> {
									promise2.fail(ex);
								});
	...
	public Future<ServiceResponse> response200SearchReportSchedule(SearchList<ReportSchedule> listReportSchedule) {
		...
		try {
			...
			json.put("list", l);
			response200Search(listReportSchedule.getRequest(), listReportSchedule.getResponse(), json);
			promise.complete(ServiceResponse.completedWithJson(Buffer.buffer(Optional.ofNullable(json).orElse(new JsonObject()).encodePrettily())));
		} catch(Exception ex) {
	...
	public Future<Void> persistReportSchedule(ReportSchedule o) {
		Promise<Void> promise = Promise.promise();
		try {
			SiteRequestEnUS siteRequest = o.getSiteRequest_();
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
			...
			sqlConnection.preparedQuery("SELECT * FROM ReportSchedule WHERE pk=$1")
				...
				try {
						...
						for(Integer i = 0; i < definition.size(); i++) {
							...
							if(!"pk".equals(columnName)) {
								try {
									...
									LOG.error(String.format("persistReportSchedule failed. "), e);
								}
							}
						}
					}
					...
					LOG.error(String.format("persistReportSchedule failed. "), ex);
					promise.fail(ex);
				}
			}).onFailure(ex -> {
				RuntimeException ex2 = new RuntimeException(ex);
				LOG.error(String.format("persistReportSchedule failed. "), ex2);
				promise.fail(ex2);
			});
		} catch(Exception ex) {
			LOG.error(String.format("persistReportSchedule failed. "), ex);
	...
	public Future<Void> relateReportSchedule(ReportSchedule o) {
		...
		try {
			...
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
			Long pk = o.getPk();
			sqlConnection.preparedQuery("SELECT typeKey as pk1, 'typeKey' from ReportSchedule where pk=$1 UNION SELECT pk as pk2, 'narrativeKeys' from ReportNarrative where scheduleKey=$2 UNION SELECT pk as pk2, 'eventKeys' from ReportEvent where scheduleKey=$3 UNION SELECT pullOwnerKey as pk2, 'pullOwnerKey' from ReportSchedule where pk=$4 UNION SELECT finalOwnerKey as pk2, 'finalOwnerKey' from ReportSchedule where pk=$5")
					.collecting(Collectors.toList())
					.execute(Tuple.of(pk, pk, pk, pk, pk)
					).onSuccess(result -> {
				try {
					if(result != null) {
						for(Row definition : result.value()) {
